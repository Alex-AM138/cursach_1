...
def test_cards_collector(value: list, expected: list) -> None:
    assert views.cards_collector(pd.DataFrame(value)) == expected
...
def test_top_transactions(value: list, expected: list) -> None:
    assert views.top_transactions(pd.DataFrame(value)) == expected


def test_main_page():
    date = datetime.datetime.strptime("28.12.2021 00:00:00", "%d.%m.%Y %H:%M:%S")
    utils.get_currency_rates = Mock(
        return_value=[{"currency": "EUR", "rate": 102.55}, {"currency": "USD", "rate": 97.31}]
    )
    utils.get_stock_prices = Mock(
        return_value=[
    ...
    assert json.loads(views.main_page(date)) == {
        "cards": [
            {"cashback": 0, "last_digits": "5091", "total_spent": 14034.37},
            {"cashback": 0, "last_digits": "7197", "total_spent": 20427.45},
            {"cashback": 0, "last_digits": "4556", "total_spent": 952.9},
        ],
        "currency_rates": [{"currency": "EUR", "rate": 102.55}, {"currency": "USD", "rate": 97.31}],
        ...
        "top_transactions": [
            ...
            {"amount": 1721.38, "category": "Каршеринг", "date": "12.12.2021", "description": "Ситидрайв"},
            {
                "amount": 1198.23,
                "category": "Переводы",
                "date": "21.12.2021",
